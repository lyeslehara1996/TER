...
std::vector<RawImageInfo> images = {
  { "IRM_5_labels_8_bits_256x256_crane.raw", 256, 256, 1, false, false}, 
  { "IRM_5_labels_8_bits_256x256_crane.raw", 256, 256, 1, true, false}, 
  
  { "IRM_8_bits_256x256_coeur.raw", 256, 256, 1, false, false},          
  { "IRM_8_bits_256x256_coeur.raw", 256, 256, 1, true, false},           
...
// Appliquer chaque LUT sur chaque image
 for (const auto& img : images) {
  ...
  //creer un dossier pour chaque nom de l'image 
  ...
  if (img.channels == 3) {
    ...
    for (size_t i = 0; i < luts.size(); ++i) {
        auto coloredImage = applyLUT(imageGray, luts[i]);
        std::string outPath = outputDir + "/" + imageBaseName + (img.bigEndian ? "_BE" : "_LE") + "_with_" + lutNames[i] + ".ppm";
        savePPM(coloredImage, img.width, img.height, outPath);
    }
}
  //  Image scalaire (1 canal)
  //uint16_t
  if (img.isUint16) {
      ...
      for (size_t i = 0; i < luts.size(); ++i) {
          auto coloredImage = applyLUT(image, luts[i]);
          std::string outPath = outputDir + "/" + imageBaseName + (img.bigEndian ? "_BE" : "_LE") + "_with_" + lutNames[i] + ".ppm";
          savePPM(coloredImage, img.width, img.height, outPath);
      }
