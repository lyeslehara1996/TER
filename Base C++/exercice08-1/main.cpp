...
const int size = 10;
//ca veut dire (&arr)[size] passer en refference un tableux |  ca int arr[] est meme que int* arr mais risque de perdre l’info de la taille du tableau à la compilation
...
// tableau statique  type c (Template)
template<typename T, size_t N>
void randomArrayC(T (&arr)[N]) {
    for (size_t i = 0; i < N; ++i) {
        arr[i] = static_cast<T>(std::rand() % 10);
    }
}
